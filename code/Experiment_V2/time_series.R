...
jan_2007 <- seismic_all %>% filter(year == 2007, month == "Jan")
nrow(jan_2007)
range(jan_2007$datetime_nz)
...
# 16 distinct colours for 2007--2022
set.seed(42)  # for reproducibility
pal <- createPalette(16, seedcolors = c("#440154", "#22a884", "#fde725"))
names(pal) <- as.character(2007:2022)
...
ggplot(seismic_all,
  ...
  labs(
    title = "Annual Displacement Time Series (2007–2022)",
    x = "Day of Year",
...
ggplot(seismic_all,
  ...
  scale_colour_manual(values = year_colours) +
  labs(
    title = "Annual RSAM Time Series (2007–2022)",
    x = "Day of Year",
    y = "RSAM",
    colour = "Year"
  ) +
...
# Yearly RSAM using facets(log scale)
ggplot(seismic_all,
       aes(x = doy,
           y = rsam_avg,
           colour = year,
