...
N_LABEL <- 12
YEAR_LABEL <- "2007 to 2019" 

results_dir <- "C:/Users/UserA1/Documents/GitHub/Seismic_Amplitude_Timeseries_Analysis/results/Weekly_OP_Analysis_2007to2019"

out_dir_facet   <- file.path(results_dir, "HC_Plane_Plots_2007to2019")
out_dir_shannon <- file.path(results_dir, "HC_Plane_Plots_single_Shannon_2007to2019")
dir.create(out_dir_facet,   recursive = TRUE, showWarnings = FALSE)
dir.create(out_dir_shannon, recursive = TRUE, showWarnings = FALSE)

input_files <- c(
  rsam_avg = file.path(results_dir, "weekly_entropy_complexity_rsam_avg_2007to2019.csv")
)
...
## ============================================================================
## PART 1: FACETED 4-panel plot (Shannon, Tsallis, Renyi, Fisher together)
## ============================================================================
...
for (var_name in names(input_files)) {
  ...
  var_label <- paste("RSAM", YEAR_LABEL)
  ...
  ## Two sheets: one for the facet plot's labelled points (all 4 measures,
  ## N_LABEL per measure), one for the Shannon-only plot's labelled points
  ## (N_LABEL total). Same underlying points in both cases -- Shannon's
  ## rows will match between sheets -- but kept separate since they come
  ## from the two different plots you asked to save.
  ...
  excel_path <- file.path(results_dir, sprintf("HC_extreme_weeks_%s_%s_D%d.xlsx", var_name, YEAR_LABEL, D))
