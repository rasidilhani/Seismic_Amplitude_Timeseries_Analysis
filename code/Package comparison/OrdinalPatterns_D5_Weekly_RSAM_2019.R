...
# Only RSAM is run through the ordinal pattern pipeline.
variables <- c(
  RSAM = "rsam_avg"
)
...
# Year is derived from the NZ-local timestamp, not the filename — same
# reasoning as in the Shiny app: deriving "year" from the filename would put
# late-Dec UTC readings (which land on Jan 1 NZ time) into the wrong bucket.
seismic_all$year <- year(seismic_all$datetime_nz)

# Week-of-year, derived from the NZ-local timestamp. lubridate::week() counts
# Jan 1 - Jan 7 as week 1, Jan 8 - Jan 14 as week 2, etc., entirely within the
# calendar year — this avoids the ISO-week edge case (isoweek()) where the
# first/last few days of the year can be assigned to a week that nominally
# belongs to the adjacent year.
seismic_all$week <- sprintf("W%02d", week(seismic_all$datetime_nz))
...
# One row per Year x Variable x Week (W01..W53).

# --- Timing starts here ---------------------------------------------------
t_start      <- Sys.time()
t_proc_start <- proc.time()
...
for (yr in years_to_use) {
  ...
  for (var_label in names(variables)) {
    ...
    week_levels <- sort(unique(year_df$week))
    
    for (wk in week_levels) {
      sub_df <- year_df %>% filter(week == wk)
      res <- run_one_period(sub_df, var_col, D, z_alpha)
      all_results[[length(all_results) + 1]] <- res %>%
        mutate(Year = yr, Variable = var_label, Period_Type = "Weekly",
               Period = wk, .before = 1)
    }
  }
}

period_order <- sprintf("W%02d", 1:53)

ordinal_results <- bind_rows(all_results) %>%
  mutate(Period = factor(Period, levels = period_order)) %>%
  arrange(Variable, Year, Period) %>%
  mutate(Period = as.character(Period))

# --- Timing ends here ------------------------------------------------------
t_proc_end <- proc.time()
t_end      <- Sys.time()

elapsed_sys  <- as.numeric(difftime(t_end, t_start, units = "secs"))
elapsed_proc <- as.numeric((t_proc_end - t_proc_start)["elapsed"])

# Attach timing to every row so it's saved alongside the results.
ordinal_results$Elapsed_secs_SysTime  <- elapsed_sys
ordinal_results$Elapsed_secs_ProcTime <- elapsed_proc

# ── Save everything as one combined, long-format workbook ─────────────────
dir.create(here("Results", "Package_comparison_results"), recursive = TRUE, showWarnings = FALSE)
out_path <- here("Results", "Package_comparison_results", "WIZ_OrdinalPatterns_D5_Weekly_RSAM_2019.xlsx")
...
cat("Elapsed (Sys.time):", elapsed_sys, "seconds\n")
cat("Elapsed (proc.time, 'elapsed'):", elapsed_proc, "seconds\n")

################################################################################
library(tidyverse)
library(lubridate)
library(ordinalpatterns)   # remotes::install_github("labepi/ordinalpatterns")
library(writexl)
library(here)
...
# ── Helper function ──────────────────────────────────────────────────────
# NOTE: the ordinalpatterns package (labepi/ordinalpatterns) only exposes
# Shannon-based permutation entropy, Shannon-based statistical complexity,
# and Fisher information. It does not provide Renyi entropy, Tsallis
# entropy, or any variance/semi-length (CI) functions, so those columns are
# intentionally left out of this version of the pipeline.
#
# Also note: fisher_information() here uses a different formula
# (sqrt-difference based) than the Fisher_Ferri() function you've used with
# StatOrdPattHxC — the two Fisher values are NOT expected to match numerically.

# Computes the confirmed-available H/C/Fisher metrics for one numeric
# series at embedding dimension D using the ordinalpatterns package.
compute_HC_op <- function(series, D, tau, n_i) {
  xpd   <- op_pd(series, D = D, tau = tau)
  probs <- xpd$probabilities
  
  Hs <- permutation_entropy(probs, normalized = TRUE)
  Cs <- statistical_complexity(probs, entropy = Hs, normalized = TRUE)
...
# Pulls one variable's series out of a (year + week)-filtered data frame,
# drops non-finite values, and guards against periods that are empty or too
# short to support D = 5 (op_pd needs at least D points to form one pattern).
run_one_period_op <- function(df, var_col, D, tau) {
  series <- df[[var_col]]
  series <- series[is.finite(series)]
  n_i <- length(series)
  
  if (!check_parameters(n = n_i, D = D, tau = tau, num = 1)) {
    return(tibble(
      N_points = n_i,
      H_Shannon = NA, C_Shannon = NA, FisherInfo = NA
    ))
  }
  
  compute_HC_op(series, D, tau, n_i)
}
...
for (yr in years_to_use) {
  ...
  for (var_label in names(variables)) {
    ...
    for (wk in week_levels) {
      sub_df <- year_df %>% filter(week == wk)
      res <- run_one_period_op(sub_df, var_col, D, tau)
      all_results[[length(all_results) + 1]] <- res %>%
        mutate(Year = yr, Variable = var_label, Period_Type = "Weekly",
               Period = wk, .before = 1)
    }
...
# ── Save everything as one combined, long-format workbook ─────────────────
dir.create(here("Results", "Package_comparison_results"), recursive = TRUE, showWarnings = FALSE)
out_path <- here("Results", "Package_comparison_results", "WIZ_OrdinalPatterns_D5_Weekly_RSAM_2019_ordinalpatterns_pkg.xlsx")
